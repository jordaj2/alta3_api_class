...
def sshlogin(ip, un, passw):
    ...
    days = (int(float(sshresult)))  # convert uptime in sec to days
    sshsession.close()
    print(days)
    return days
...
@app.route("/graphin")
def graphin():
    with open("/home/jordaj2/sshpass.yml") as sshpass: # creds for our servers
    ...
    plt.yticks(np.arange(0, 6000000, 250000)) # prob want to turn this into a log scale
    plt.legend(['uptime'])
